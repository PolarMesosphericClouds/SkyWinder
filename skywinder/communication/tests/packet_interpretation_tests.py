...
class test_sip_packet_malfunctions(unittest.TestCase):
    ...
    def sip_packet_in_data_test(self):
        initial_buffer = '\xfa\xff\x01\x00\x00\x00\x01'
        np.random.seed(0)
        random_msg = np.random.randint(0, 255, size=10000).astype('uint8').tostring()
    ...
    def false_start_test(self):
        ...
        assert (packets[0].to_buffer() == '\xfa\xff\x01\x00\x00\x00\x01')
    ...
    def insufficient_length_for_length_byte_test_2(self):
    ...
    def insufficient_length_for_checksum_test(self):
    ...
    def bad_id_byte_test(self):
        ...
        packets, remainder = packet_classes.get_packets_from_buffer(buffer,
                                                                    packet_class=packet_classes.GSEPacket,
                                                                    start_byte=packet_classes.GSEPacket.START_BYTE)
        assert (remainder == '\x99\x01\x00\x00\x00\x01')
        assert (packets == [])
    ...
    def bad_checksum_test(self):
        ...
        packets, remainder = packet_classes.get_packets_from_buffer(buffer,
                                                                    packet_class=packet_classes.GSEPacket,
                                                                    start_byte=packet_classes.GSEPacket.START_BYTE)
        assert (remainder == '\xff\x01\x00\x00\x00\x07')
        assert (packets == [])
    ...
    def long_sip_packet_test(self):
        ...
        assert (checksum < 256)
        buffer = np.array(sync + header + length + data + [checksum], dtype='uint8').tostring()
        packets, remainder = packet_classes.get_packets_from_buffer(buffer,
                                                                    packet_class=packet_classes.GSEPacket,
                                                                    start_byte=packet_classes.GSEPacket.START_BYTE)
        assert (remainder == '')
...
class TestFilePackets(unittest.TestCase):
    def setUp(self):
        ...
        self.random_msg = self.random_msg.tostring()
        self.random_msg = '\x02' + self.random_msg[1:]
