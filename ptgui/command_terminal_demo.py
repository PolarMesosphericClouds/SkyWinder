...
from pmc_turbo.communication.command_table import(DESTINATION_ALL_CAMERAS,
...
class ExampleWidget(QWidget):
    ...
    def __init__(self, command_sender_app, parent=None):
        ...
        ipyConsole.pushVariables(dict(quick_shell_command=self.quick_shell_command,
                                      quick_set_leader=self.quick_set_leader))
    ...
    def quick_shell_command(self,command,destination,flush=False, timeout=10, max_num_bytes_returned=2**20):
        if flush:
            flush_command = command_table.command_manager.flush_downlink_queues()
        else:
            flush_command = ''
        return self.cs.send(flush_command +
                            command_table.command_manager.run_shell_command(command_line=command,timeout=timeout,
                                                                            max_num_bytes_returned=max_num_bytes_returned),
                            destination=destination)
    def quick_set_leader(self,leader_id):
        return self.cs.send(command_table.command_manager.set_leader(leader_id=leader_id),destination=DESTINATION_SUPER_COMMAND)
